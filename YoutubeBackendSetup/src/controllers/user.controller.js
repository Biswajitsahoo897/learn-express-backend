...
const getUserChannelProfile = asyncHandler(async (req, res) => {
    ...
    const channel = await User.aggregate([{
        ...
        $lookup: {
            from: "subscriptions",
            // converted in to the lowercase and plural (came from the subscription Model)
            localField: "_id",
            foreignField: "channel",
            as: "subscribers"
        }
        // look for the subscriber & performs a left outer join with another collection
    },
    {
        $lookup: {
            from: "subscriptions",
        ...
        // how many subscriber u got through the channel
    },
    {
        $addFields: {
            subscribersCount: {
            ...
            // $in: checks if req.user._id is in the list of subscriber IDs
            isSubscribed: {
                $in: [
                    req.user._id, // The current logged-in user ID
                    {
                        $map: {
                            input: "$subscribers",           // loop over each subscriber object
                            as: "sub",                        // name each loop variable as `sub`
                            in: "$$sub.subscriber"            // extract `subscriber` field from each
                        }
                    }
                ]
            }
